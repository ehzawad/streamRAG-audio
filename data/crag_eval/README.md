...
full pages produce 1,514 chunks under the current 256/32 indexing contract. No
page is shortened.

## Status

`candidate_pending_human_review` means the dataset is proposed but not yet
human-approved. Review every item and the corpus checklist in `REVIEW_SHEET.md`
without viewing either path's predictions. There is no separate freeze ceremony
in the shipped tooling: set `ALLOW_UNREVIEWED_DATASET=1` to run local checks
against this candidate corpus, and do not label those runs final until the
review is complete.
